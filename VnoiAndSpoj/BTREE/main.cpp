...
ll NumOfSubseqForVector(const vector<int>& s) {
    memset(dp, 0, sizeof(dp));
    memset(last, -1, sizeof(last));
    int l = s.size();
    dp[0] = 1;
...
ll Solution() {
    ...
    while (i < lt) {
        ...
        if ((cnt > 0) && (curS.size() > 0))
            ans = (ans + (((NumOfSubseqForVector(curS) - 1LL + Mod)%Mod)*max(CurHeight, cnt))%Mod)%Mod;
        cnt = 0;
        while ((i < lt) && (t[i] == 'U')) {
            i++;
            cnt++;
    ...
    ans = (ans + min(CurHeight, TotalNumOfU))%Mod;
    return ans;
}
