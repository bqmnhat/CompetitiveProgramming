...
ld disToSegemnt (Point x, Point m, Point n){
    Vec mn, mx, nx;
    mn.buildVector (m, n);
    mx.buildVector (m, x);
    nx.buildVector (n, x);
...
int main(){
    ...
    fout << setprecision(5)
            << min(disToSegemnt(k, a[closestPointIndex], a[adjacent_closest1_index]),
                                   disToSegemnt(k, a[closestPointIndex], a[adjacent_closest2_index]));
    return 0;
}
