...
void DFSMakeNewGraph(int x, ll SumU, ll SumV, int PrevMarked) {
    if (IsStudent[x]) {
        if (PrevMarked != -1) {
    ...
    for (auto ty: g[x])
        if ((!vis[ty.y]) && (!cal[ty.y]))
            DFSMakeNewGraph(ty.y, SumU + ty.u, SumV + ty.v, PrevMarked);
}
...
void FindHamiltonPath(int x, int cnt, ll cost) {
    if (cost > ans)
        return;
    if ((cnt == k+3) && (x == n)) {
        ans = min(ans, cost);
    ...
    for (auto ty: NewG[x]) {
        if (!vis[ty.y]) {
            vis[ty.y] = true;
            FindHamiltonPath(ty.y, cnt+1, cost + ty.u + ty.v*1LL*cnt);
            vis[ty.y] = false;
        }
    }
}
